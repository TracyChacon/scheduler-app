// src/components/StagePipeline.tsx
"use client"
...
// Explicit professional role clearances mapped directly to your preset domains
...
export default function StagePipeline({
    stages,
    clients,
    userRole = 'Mortgage Loan Officer', // Defensively fall back to a standard baseline role
    canDragAndDrop,
    onUpdatePipelineStructure,
    onMoveClient,
}: StagePipelineProps) {
    // --- SECURITY ACCESS ASSIGNMENT FLAGS ---
    const isSystemAdmin = userRole === 'admin';
    
    // All specific professions + administrative keys have layout drag permissions
    const hasDataClearance = [
        'admin', 
        'Mortgage Loan Officer', 
        'Loan Officer Assistant', 
        'Underwriter', 
    ...
    // --- STATE LAYER FOR TRACKING DRAFT EDITS BEFORE PERSISTENCE ---
    ...
    // --- TRANSACTION OPERATIONAL LIFECYCLE ---
    ...
    const executeStageRemovalConfirm = () => {
        ...
        const adjustedClients = draftClients.map(client => 
            client.stage === stageToRemove 
            ? { ...client, stage: fallbackStageId} 
            : client
        );
    ...
    return (
        <div className='space-y-4 text-slate-100'>
            ...
            <div className='bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
                ...
                {/* ADMIN CONFIGURATION SECTORS INTERFACE PANEL */}
                {isSystemAdmin && (
                        ...
                        {isEditMode ? (
                            <div className="flex gap-2 border-l border-slate-800 pl-2">
                                <button
                                    ...
                                    className="bg-slate-950 border border-slate-800 hover:bg-slate-900 text-slate-400 font-bold px-4 py-1.5 rounded-lg text-xs uppercase tracking-wide cursor-pointer"
                                >
                                    Cancel
                                </button>
                                ...
                                <button
                                    onClick={handleSave}
                                    className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black px-4 py-1.5 rounded-lg text-xs uppercase tracking-wide cursor-pointer shadow-lg shadow-emerald-500/20"
                                >
                                    Save Changes
                                </button>
                            </div>
                            ...
                            <div className='relative'>
                                <button
                                    ...
                                    className='bg-slate-950 border border-slate-800 text-slate-400 hover:text-slate-200 font-bold p-2 rounded-lg text-sm transition-all cursor-pointer flex items-center justify-center h-9 w-9 hover:bg-slate-800'
                                    title="Open Pipeline Configuration Options"
                                >
                                    ⚙️
                                </button>
                                ...
                                {isMenuOpen && (
                                    <div className='absolute right-0 mt-2 top-full flex flex-col gap-1.5 bg-slate-950 border border-slate-800 rounded-xl p-2 shadow-2xl z-50 min-w-[160px] animate-fadeIn'>
                                        <button
                                            ...
                                            className='w-full text-left bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-slate-950 font-bold px-3 py-2 rounded-lg text-xs tracking-wide uppercase cursor-pointer transition-all'
                                        >
                                            ➕ Add Stage
                                        </button>
                                        ...
                                        <button
                                            ...
                                            className="w-full text-left bg-rose-500/10 hover:bg-rose-500 text-rose-400 hover:text-slate-950 font-bold px-3 py-2 rounded-lg text-xs tracking-wide uppercase cursor-pointer transition-all"
                                        >
                                            ➖ Remove Stage
                                        </button>
                                        ...
                                        <button
                                            ...
                                            className="w-full text-left bg-violet-500/10 hover:bg-violet-500 text-violet-400 hover:text-slate-950 font-bold px-3 py-2 rounded-lg text-xs tracking-wide uppercase cursor-pointer transition-all"
                                        >
                                            🗘 Reorder Stage
                                        </button>
                                        ...
                                        <button
                                            ...
                                            className="w-full text-left bg-amber-500/10 hover:bg-amber-500 text-amber-400 hover:text-slate-950 font-bold px-3 py-2 rounded-lg text-xs tracking-wide uppercase cursor-pointer transition-all"
                                        >
                                            𝜟 Rename Stage
                                        </button>
                                    </div>
            ...
            {/* DROPDOWN DETOUR MODAL PANEL FOR CLIENT ROUTING ON COLUMN REMOVAL */}
            {stageToRemove && isSystemAdmin && (
                <div className="bg-slate-900 border-2 border-rose-500/30 rounded-xl p-5 space-y-4 max-w-xl animate-slideDown">
                    <div>
                        <h4 className="text-sm font-bold text-slate-200 uppercase tracking-wide">Route Displaced Leads</h4>
                        <p className="text-xs text-slate-400 mt-0.5">
                            Select the pipeline stage to move clients to from the deleted stage:
                        </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-3 mt-1">
                        <select
                            ...
                            className="bg-slate-950 border border-slate-600 rounded-lg text-xs p-2 text-slate-300 outline-none min-w-[200px]"
                        >
                        ...
                        <div className="flex gap-2">
                            <button
                                ...
                                className="px-3 py-1.5 bg-slate-950 border border-slate-800 text-xs text-slate-400 rounded-lg hover:text-slate-200 cursor-pointer font-bold"
                            >
                                Back
                            </button>

                            <button
                                onClick={executeStageRemovalConfirm}
                                className="px-3 py-1.5 bg-rose-500 hover:bg-rose-400 text-slate-950 font-bold text-xs rounded-lg cursor-pointer"
                            >
                                Confirm Deletion
                            </button>
                        </div>
                ...
                {currentDisplayStages.map((stage) => {
                    ...
                    return (
                        <div
                            ...
                            onDragStart={(event) => handleDragStart(event, stage.id, 'stage')}
                            onDragOver={handleDragOver}
                            onDragEnter={handleDragEnter}
                            onDrop={(event) => handleDropOnStageColumn(event, stage.id)}
                            className={`bg-slate-950 border rounded-lg p-3 flex flex-col min-h-36 justify-between transition-all ${
                                isReorderActive
                                    ? "border-violet-500/40 bg-slate-950/80 cursor-ew-resize border-dashed"
                                    : "border-slate-800 hover:border-slate-700"
                            }`}
                        >
                            {/* Inner wrapper blocks clicks/drags from getting swallowed by child elements */}
                            <div className="space-y-2 pointer-events-none">
                                {/* Stage Heading Header Block */}
                                <div className="flex flex-row items-start justify-between border-b border-slate-800 pb-1.5 min-h-10 gap-1">
                                    ...
                                    <div className="pointer-events-auto flex gap-1">
                                        {isEditMode && activeAction === 'remove' && currentDisplayStages.length > 1 && isSystemAdmin && (
                                            <button
                                                onClick={() => initiateStageRemoval(stage.id)}
                                                className='flex items-center justify-center text-red-500 hover:text-red-400 border border-red-500/40 w-4 h-4 rounded-full text-[9px] font-black cursor-pointer bg-red-500/5 hover:bg-red-500/10'
                                            >
                                                ✕
                                            </button>
                                        )}
                                        ...
                                        {isEditMode && activeAction === 'rename' && isSystemAdmin && (
                                            <button
                                                onClick={() => handleRenameStageClick(stage.id)}
                                                className='text-amber-500 hover:text-amber-400 text-[10px] font-bold px-1 cursor-pointer'
                                                title="Rename Stage"
                                            >
                                                ✎
                                            </button>
                                ...
                                <div className="space-y-1.5">
                                    {associatedClients.map((client, idx) => {
                                        ...
                                        return (
                                            <div
                                                key={idx}
                                                draggable={allowDragCard}
                                                onDragStart={(event) => {
                                                    event.stopPropagation(); // Avoid triggering column reorder handlers
                                                    handleDragStart(event, client.name, 'client');
                                                }}
                                                className={`px-2 py-1 border rounded text-[11px] font-medium truncate select-none transition-all pointer-events-auto ${
                                                    allowDragCard
                                                        ? "bg-slate-900 border-slate-800 text-emerald-400 cursor-grab active:cursor-grabbing hover:border-emerald-500/30"
                            ...
                            <span className="text-[10px] font-mono text-slate-600 mt-2 self-end pointer-events-none">
                                Qty: {associatedClients.length}
                            </span>
                        </div>
                    );
                })}
